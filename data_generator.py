...
class DataGenerator(object):
    ...
    def generate_ball_batch(self, train=True):
        # Note train arg is not used.

        xs = np.zeros([self.batch_size, self.num_samples_per_class, self.dim_input])
        ys = np.zeros([self.batch_size, self.num_samples_per_class, self.dim_output])

        for i in range(self.batch_size):
            # for each task (different m1, m2, cr (coefficient of restitution))
            m1 = np.random.uniform(self.m_range[0], self.m_range[1])
            m2 = np.random.uniform(self.m_range[0], self.m_range[1])
            cr = np.random.uniform(self.cr_range[0], self.cr_range[1])
            N_norm = np.random.uniform(self.N_range[0], self.N_range[1])
